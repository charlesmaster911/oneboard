...
const LOCAL_MANUAL_DOCUMENTS = Object.freeze([Object.freeze({
  ...
  content: [
    ...
    '## 전달받은 엑셀 사용 판정',
    '엑셀 사용 판정: 조건부 사용 가능. OneBoard에 파일 자체를 업로드하는 형식은 아니며, OWNER가 채널별 입력칸에 한 번씩 옮겨 저장할 때만 사용합니다.',
    '- 바로 대응: 카페24 3개 항목, 네이버 스마트스토어 2개 항목, 네이버 검색광고 3개 항목.',
    '- META: App Secret으로 적힌 긴 EAA 형식 값은 Access Token일 가능성이 높습니다. 원 플랫폼에서 확인한 뒤 Access Token 칸에 넣고, 실제 App Secret은 별도로 확인합니다. 광고계정 ID는 act_로 시작하는 전체 값을 사용합니다.',
    '- 쿠팡: 엑셀에 쿠팡 계정이 2세트 있습니다. OneBoard는 한 세트만 저장하므로 WING에서 한반도 계정의 Vendor ID를 확인한 뒤 일치하는 한 세트만 입력합니다.',
    '- 카카오모먼트: access_token은 카카오 비즈니스 인증으로 발급한 Business Token인지 확인한 뒤 사용합니다.',
    '- 보안: 엑셀에는 비밀키가 평문으로 들어 있으므로 팀 업무·회의록·메일·공유 Drive에 올리지 않습니다. OneBoard 저장과 수집 성공 후 외부 공유 이력이 있으면 모든 키를 재발급하고 기존 파일을 폐기합니다.',
    '## 채널별 발급 위치',
